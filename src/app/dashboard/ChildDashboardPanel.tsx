...
export default function ChildDashboardPanel({
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {child.weeks.map((tile) => (
            <div key={tile.week} className={weekTileClasses(tile.status)}>
              ...
              <div className="text-[11px] font-semibold mt-1.5 uppercase tracking-wide">{weekStatusLabel(tile.status)}</div>
              {weekActions(tile)}
            </div>
          ))}
        </div>
      ...
      {showIncremental && incrementalChild && (
        <section className="bg-[#fffaf5] border border-[#f0e2d9] rounded-2xl p-5">
          <h3 className="text-base font-bold text-[#062744] mb-1">Add more camp weeks</h3>
          <p className="text-sm text-slate-600 mb-4">
            While your family registration is still pending payment, you can select additional weeks that have openings.
          </p>
          {showPaidFamilyBanner && (
          ...
          <div className="grid sm:grid-cols-2 gap-2 text-sm">
            {CAMP_SESSIONS.map((week) => {
              const held = incrementalChild.existingWeeks.some((e) => e.week === week)
              const remaining = weekRemaining[week] ?? 0
              const canAdd = incrementalChild.submissionPending && !held && remaining > 0
              const isSelected = extraWeekSelection.has(week)
              const existing = incrementalChild.existingWeeks.find((e) => e.week === week)
              return (
                <label
                  key={week}
                ...
                >
                  <input
                    ...
                    checked={held || isSelected}
                    disabled={held || !canAdd}
                    onChange={() => onToggleExtraWeek(week, !!canAdd)}
                  />
                  <span>
                    ...
                    {held && existing && (
                      <span className="text-xs block mt-0.5 text-slate-500">
                        Already reserved (
                        {existing.displayStatus === 'confirmed'
                          ? 'paid/confirmed'
                    ...
                    {!held && !canAdd && incrementalChild.submissionPending && (
          ...
          {canSubmitIncremental && (
            <button
              ...
              className="mt-4 w-full sm:w-auto bg-[#062744] hover:bg-[#041f36] disabled:opacity-50 text-white font-bold py-3 px-8 rounded-full text-sm transition-colors"
            >
